...
calIt();paintScore();$("#ardoraTab").css("visibility","hidden");
$(".draggable").draggable({revert: "invalid"});
$("#drop").droppable({accept: ".draggable",drop: function(event, ui){var dropped=ui.draggable;var droppedOn = $(this);var className=$(dropped).attr("src").substr($(dropped).attr("src").lastIndexOf("/")+1);className=className.substr(0,className.lastIndexOf("."));$(dropped).removeClass("c"+className);$(dropped).addClass("rec");$(dropped).detach().css({top:0,left:0}).appendTo(droppedOn);calIt();paintScore();}});
$("#drop").sortable();
...
var className=$(dropped).attr("src").substr($(dropped).attr("src").lastIndexOf("/")+1);className=className.substr(0,className.lastIndexOf("."));$(dropped).addClass("c"+className);calIt();paintScore();}});
